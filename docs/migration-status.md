...
pnpm check                   # prettier + astro check + cms/schema parity + tsc -b packages/cdk + cms-auth tests
pnpm format
```
...
Search/social metadata, a daily canary that emails on failure, and the docs.
All of it verified against the live test domain, not just the build output.

- **`src/components/Seo.astro`** — one head component for every page that uses
  `Base`: title, description, canonical, `theme-color` (`#172554`), the full
  Open Graph set with a generated 1200×630 JPEG, and the Twitter
  `summary_large_image` card. Pages no longer write meta tags themselves.
- **`src/components/JsonLd.astro`** — schema.org `NGO` on the home page,
  assembled from `site.yml` and `contacts.yml`. Nothing is hard-coded: the
  contact email is looked up by `role: President`, and the postal address is
  parsed out of `site.donate.mail`. An editor changing the board in the CMS
  changes the structured data.
- **A favicon set** — `favicon.svg` (the mark), `favicon.ico` (16/32/48) and
  `apple-touch-icon.png` (180, flattened; iOS ignores alpha), generated by the
  dev-only `packages/app/scripts/favicon.mjs`.
- **A daily canary and an SNS alarm** in `site-stack.ts`, plus
  `packages/cdk/canary/index.js`.
- **Docs** — `README.md` (new, the public front door), `docs/editing.md`
  finalized, `CLAUDE.md` extended with the SEO, CMS-auth and monitoring
  sections.

### Two SEO defects the site was already shipping

Both were found by fetching the live test domain rather than by reading the
code, and both are consequences of `build.format: 'file'`:

- **The home page canonical was `https://…/index.html`.** With
  `build.format: 'file'` the index page's own `Astro.url.pathname` is literally
  `/index.html`, so the obvious `new URL(Astro.url.pathname, Astro.site)`
  produces a URL nobody visits. `Seo.astro` now strips a trailing `index.html`
  for the canonical and `og:url`, and leaves every other `.html` path alone —
  `/concert.html` _is_ the visited URL.
- **The sitemap advertised `/concert`, which 404s.** `@astrojs/sitemap` does
  not know about `build.format: 'file'` and drops the extension from every URL
  it lists. `/concert` → 404, `/concert.html` → 200, verified with curl. The
  page is a `noindex` share card that redirects to `/#events`, so it is now
  excluded from the sitemap entirely. If a future `.html` page ever does belong
  in the sitemap, the fix is the `serialize` option, not `filter`.

The live sitemap is now a single entry, `https://<domain>/`.

### What the canary checks, and how to change it

`hrs-<stage>-daily`, from `packages/cdk/canary/index.js`, at **13:00 UTC**
daily (`cron(0 13 * * ? *)` — roughly 08:00 Central; `rate()` cannot express
once a day). Three steps, each of which screenshots into the artifacts bucket:

| Step             | Assertion                                             |
| ---------------- | ----------------------------------------------------- |
| `navigate`       | `GET https://<stage domain>/` returns **200**         |
| `verify-heading` | the first `h1` contains **"Haitian Relief Services"** |
| `verify-gallery` | at least one **`.gallery img`** exists                |

Every failure message states expected vs. actual — the forced failure below
read `Expected HTTP 200 loading …/deliberately-broken-canary-test.html, got
404`, which is diagnosable from a phone.

**`.gallery img` is a contract with `ProjectGallery.astro`.** Rename that
container class and the canary starts failing at 8am rather than at build time.

**To change the alert email**: edit the `EmailSubscription` in
`site-stack.ts` and deploy. SNS then sends the new address a subscription
confirmation that a human has to click; until then, alarms go nowhere. Each
stage has its own topic (`hrs-<stage>-alerts`), so **prod cutover will send a
fresh confirmation email that must be clicked**.

### Things about the canary that are easy to get wrong

- **`artifactsBucketLifecycleRules` on the `Canary` construct silently does
  nothing** once `artifactsBucketLocation` is passed. The 30-day expiry lives
  on the explicit `CanaryArtifactsBucket` instead. The bucket is explicit so
  non-prod teardown in Phase 6 doesn't orphan it (`RETAIN` is CDK's S3
  default).
- **`canary/index.js` is CommonJS** even though `packages/cdk` is
  `"type": "module"`. It is zipped and run inside the Synthetics Lambda
  runtime, never loaded by local Node.
- **The Playwright runtimes want the handler at the asset root** (`index.js`),
  not the `nodejs/node_modules/` layout the Puppeteer runtimes require. CDK
  validates this at synth time.
- The module is **`@aws/synthetics-playwright`**, not `@amzn/...` — some AWS
  doc pages still say the latter. Confirmed by a real run, not by reading.
- `Cleanup.LAMBDA` is deprecated in aws-cdk-lib 2.268.0; `provisionedResource
Cleanup: true` replaces it, and the construct throws if both are set.
- CDK 2.268.0's newest Playwright enum is `SYNTHETICS_NODEJS_PLAYWRIGHT_6_0`.
  The account offers up to `syn-nodejs-playwright-8.0`, but CDK can only select
  a runtime it has a constant for.

### The alarm's timing, measured

`SuccessPercent < 100`, `Average`, **one period of one day**,
`treatMissingData: BREACHING`, both the alarm and OK actions on the topic.

The daily period turns out to be a **rolling 24 hours ending now**, not a
calendar day — CloudWatch's own state reason named the window as
`[Sep 6 15:16 → Sep 7 15:16]`. That matters: it means the alarm does **not**
...
Against **https://sveltia.haitianrelief.org** and the real AWS resources.

- **The canary ran green against the live site.** Real run (not a dry run),
  `PASSED` in 18s, all three steps succeeded, six step screenshots plus
  `SyntheticsReport-PASSED.json` and a HAR in the artifacts bucket.
  `SuccessPercent = 100` published to CloudWatch.
- **Forced failure, end to end.** Repointed the canary's `SITE_URL` at
  `https://sveltia.haitianrelief.org/deliberately-broken-canary-test.html`
  (a real 404 on the real domain, so the actual status assertion is what
  fails), ran it, and watched:

  | Time (UTC) | Event                                         |
  | ---------- | --------------------------------------------- |
  | 15:19      | run `FAILED` — `Expected HTTP 200 …, got 404` |
  | 15:20:30   | alarm `OK → ALARM`, SNS notified              |
  | 15:21      | `SITE_URL` restored, run `PASSED`             |
  | 15:21:30   | alarm `ALARM → OK`, SNS notified              |

  Tyler had confirmed the SNS subscription before this, so both notifications
  were delivered rather than dropped.

- **The alarm's own evaluation window was temporarily narrowed to 60s for that
  test** and restored afterwards. A one-day window cannot show an ALARM→OK
  round trip inside a session. Threshold, comparison, statistic and both SNS
  actions were unchanged — only the period, so what was exercised is the real
  wiring from metric to inbox.
- **The stack is drift-free afterwards.** `detect-stack-drift` on
  `OrgHaitianReliefSveltia` returns `IN_SYNC`, 0 drifted resources. This was
  worth checking: **CloudFormation will not undo out-of-band changes on its
  own**, because a redeploy with an unchanged template skips unchanged
  resources. The canary schedule, code location, env vars and the alarm were
  each restored explicitly to the values in the deployed template.
- **Live head, sitemap and robots** all confirmed by fetching the deployed
  site: canonical `https://sveltia.haitianrelief.org/`, the full OG/Twitter
  set, valid JSON-LD, `favicon.svg`/`favicon.ico`/`apple-touch-icon.png` all
  200, and `concert.html` still carrying its own independent OG block.
- **The prod build was checked separately** (`STAGE=prod SITE_URL=…`):
  `robots.txt` has `Allow: /`, `Disallow: /admin/` and the `Sitemap:` line; the
  sitemap and canonical are the apex domain.
- `pnpm check` green.

### Decisions made in Phase 5

- **No CSP.** `site-stack.ts` carried a `Phase 5 may add one` TODO; the answer
  is no, and the comment now says why. The GTM bootstrap is inline, GTM injects
  more inline script, and a static S3 origin cannot mint a per-request nonce
  without Lambda@Edge — so any policy we could actually ship would need
  `script-src 'unsafe-inline'`, which is the one thing worth having a CSP for.
- **The favicon is a new mark**, an amber `H` on a blue-950 tile, in the Phase
  4 palette. The old `favicon.ico` was a 16×16-only black blob inherited from
  the previous site. Tyler approved keeping it on 2026-09-07. It is a
  monogram, not a designed logo — regenerate from `public/favicon.svg` if that
  ever changes.
- **`og:image:alt` was written from the photograph**, not from the filename:
  `media/pc2.jpg` is girls outside the Ganthier school, arm in arm, several in
  pillowcase dresses.

### Still open from Phase 5

- **Nothing blocking.**
- **Lighthouse SEO still cannot pass on the test domain** — it serves
  `Disallow: /` and `X-Robots-Tag: noindex, nofollow` by design, which caps the
  category at ~70 no matter what the metadata says. Re-measure on prod after
  cutover; every other SEO audit already passes.
- **The first scheduled canary run is 13:00 UTC**, so the schedule itself
  (as opposed to the script, the metric, the alarm and the email, all of which
